...
from .exceptions import BluetoothError, SecurityError
from .security import BlufiAES, BlufiCRC, BlufiCrypto
from .utils import *
...
class BlufiClient:
    def __init__(self,
                 blufi_service_uuid=BLUFI_SERVICE_UUID,
                 blufi_write_char_uuid=BLUFI_WRITE_CHAR_UUID,
                 blufi_notif_char_uuid=BLUFI_NOTIF_CHAR_UUID,
                 blufi_notif_desc_uuid=BLUFI_NOTIF_DESC_UUID
                 ):
        # UUID's
        self.blufi_service_uuid = blufi_service_uuid
        self.blufi_write_char_uuid = blufi_write_char_uuid
        self.blufi_notif_char_uuid = blufi_notif_char_uuid
        self.blufi_notif_desc_uuid = blufi_notif_desc_uuid
        ...
        self.mBlufiMTU = -1
        # Lock for state shared between notification callback and main thread
    ...
    def _cleanup(self) -> None:
        ...
        if self._bleak_client:
            try:
                ...
                log.debug("disconnect failed during cleanup")
        self._reset_state()
    ...
    def stopNotify(self):
        ...
        if not self._notify_en:
            ...
            return
        self.await_bleak(self._bleak_client.stop_notify(self.blufi_notif_char_uuid))
        self._notify_en = False
    ...
    def startNotify(self):
        ...
        if self._notify_en:
            ...
            return
        self.await_bleak(self._bleak_client.start_notify(self.blufi_notif_char_uuid, self.onNotify))
        self._notify_en = True

    def connectByName(self, name: str, timeout: float = None) -> bool:
    ...
    async def _connect_async_name(self, name: str, timeout: float) -> bool:
        ...
        if device is None:
            log.error("Device '%s' not found" % name)
            return False
        ...
        try:
            ...
            svc = client.services.get_service(self.blufi_service_uuid)
            self.notif_char = svc.get_characteristic(self.blufi_notif_char_uuid)
            self.write_char = svc.get_characteristic(self.blufi_write_char_uuid)
            await client.start_notify(self.blufi_notif_char_uuid, self.onNotify)
            self._notify_en = True
    ...
    def getSSIDList(self):
        with self._lock:
            return list(self.ssidList)

    def receiveAck(self, sequence, timeout=5):
        """Wait for an ACK matching the given sequence number."""
        try:
            ack = self.mAck.get(timeout=timeout)
            if ack != sequence:
                log.warning("ACK mismatch: expected %d, got %d" % (sequence, ack))
                return False
            return True
        except queue.Empty:
            log.error("ACK timeout waiting for seq %d" % sequence)
            return False
    ...
    def parseNotification(self, data):
        with self._lock:
            self._parseNotificationLocked(data)

    def _parseNotificationLocked(self, data):
        seq = int(data[2])
        self.mReadSequence += 1
        ...
        if fctl.isChecksum():
            ...
            if (respChecksum1 != calcChecksum1) or (respChecksum2 != calcChecksum2):
                ...
                log.debug("expect checksum: %d, %d" % (respChecksum1, respChecksum2))
                log.debug("received checksum: %d, %d" % (calcChecksum1, calcChecksum2))
                return
            else:
                log.info("CRC OK!")
                pass
    ...
    def negotiateSecurity(self):
        ...
        if not self.await_bleak(event_wait(self.secEvent, 5)):
            raise SecurityError('Security negotiation timed out waiting for device key exchange')
