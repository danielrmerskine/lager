...
Run with: lager python test_i2c_aardvark.py --box <boxname>

Prerequisites:
- An I2C net configured in /etc/lager/saved_nets.json with instrument "Aardvark"
- Example net configuration:
  {
    ...
    "instrument": "Aardvark",
    "pin": "I2C0",
    "address": "USB0::0x0403::0xE0D0::TPCFgWUp::INSTR"
  }

Optional: BMP280 sensor wired to Aardvark I2C for device-specific tests.
  - BMP280 I2C address: 0x76 (SDO low)
  - Chip ID register: 0xD0, expected value: 0x58
  - Power BMP280 externally (e.g., lager dac dac1 3.3 --box <YOUR-BOX>)
"""
import sys
import os
import json
...
def test_imports():
    ...
    try:
        from lager.protocols.i2c import AardvarkI2C
        _record("import AardvarkI2C", True)
    except Exception as e:
        _record("import AardvarkI2C", False, str(e))
        ok = False
...
# ---------------------------------------------------------------------------
def test_net_get():
    """Test Net.get('i2c1', NetType.I2C) returns I2CNet."""
    print("\n" + "=" * 60)
    print("TEST: Net.get")
    print("=" * 60)
...
def test_config_frequencies():
    """Test various frequencies: 10k, 100k, 400k, 800k, 1kHz."""
    print("\n" + "=" * 60)
    ...
    # Non-standard frequencies: config must be accepted without error.
    # Bus functionality depends on the target device -- BMP280 only
    # supports standard/fast mode, so we don't assert scan results.
...
def test_config_pull_ups():
    """Test config with pull_ups=True, False, None."""
    print("\n" + "=" * 60)
    print("TEST: config pull-ups")
    print("=" * 60)
    ...
    try:
        ...
        found = i2c.scan(start_addr=BMP280_ADDR, end_addr=BMP280_ADDR)
        _record("config pull_ups=True", BMP280_ADDR in found)
    except Exception as e:
        _record("config pull_ups=True", False, str(e))
        ok = False

    # Disable pull-ups (may or may not work depending on external pull-ups)
    try:
        i2c.config(frequency_hz=100_000, pull_ups=False)
        _record("config pull_ups=False", True, "accepted (bus may not work without external pull-ups)")
    except Exception as e:
        _record("config pull_ups=False", False, str(e))
        ok = False

    # None (should not change pull-up state)
    try:
        i2c.config(frequency_hz=100_000, pull_ups=None)
        _record("config pull_ups=None", True)
    except Exception as e:
        _record("config pull_ups=None", False, str(e))
        ok = False
...
# ---------------------------------------------------------------------------
def test_read_basic():
    """Test read(address, num_bytes) with various byte counts."""
    print("\n" + "=" * 60)
...
def test_read_edge_cases():
    """Test read with edge case parameters."""
    print("\n" + "=" * 60)
    print("TEST: read edge cases")
    print("=" * 60)
    ...
    # Read from address 0x00 (general call)
    try:
        data = i2c.read(address=0x00, num_bytes=1)
        _record("read from address 0x00", True, f"returned {data}")
    except Exception as e:
        _record("read from address 0x00", True,
                f"raises {type(e).__name__}: {e}")

    # Read from address 0x7F (max valid)
    try:
        data = i2c.read(address=0x7F, num_bytes=1)
        _record("read from address 0x7F", True, f"returned {data}")
    except Exception as e:
        _record("read from address 0x7F", True,
                f"raises {type(e).__name__}: {e}")
...
def test_write_edge_cases():
    ...
    try:
        original = i2c.write_read(address=BMP280_ADDR, data=[CTRL_MEAS_REG], num_bytes=1)
        i2c.write(address=BMP280_ADDR, data=[CTRL_MEAS_REG, 0xFF])
        readback = i2c.write_read(address=BMP280_ADDR, data=[CTRL_MEAS_REG], num_bytes=1)
        i2c.write(address=BMP280_ADDR, data=[CTRL_MEAS_REG, original[0]])
        _record("write 0xFF and readback", True,
                f"readback={hex(readback[0])}")
    except Exception as e:
        _record("write 0xFF", False, str(e))
        ok = False
    ...
    try:
        ...
        readback = i2c.write_read(address=BMP280_ADDR, data=[CTRL_MEAS_REG], num_bytes=1)
        passed = readback[0] == 0x00
        _record("write 0x00 and verify", passed,
                f"readback={hex(readback[0])}")
        if not passed:
            ok = False
    except Exception as e:
...
def test_write_read_basic():
    ...
    # Read calibration data (26 bytes)
    try:
    ...
    # Read status register -- may fail with bus recovery error after
    # many sequential transactions in a single process.  The device is
    # provably present (chip ID + calibration succeeded above), so a
    # bus-level I2CBackendError is acceptable here.
    try:
...
# ---------------------------------------------------------------------------
# 14. Write_read edge cases
# ---------------------------------------------------------------------------
def test_write_read_edge_cases():
    """Test write_read with edge case parameters."""
    print("\n" + "=" * 60)
    print("TEST: write_read edge cases")
    print("=" * 60)
    ...
    try:
        ...
        passed = isinstance(data, list) and len(data) == 128
...
def test_dispatcher_config():
    ...
    try:
        ...
        _record("dispatcher config 100kHz pull_ups=True", True)
    except Exception as e:
...
def test_dispatcher_overrides():
    ...
    # Restore defaults
    from lager.protocols.i2c.dispatcher import config
    config(I2C_NET, frequency_hz=100_000, pull_ups=True, overrides=None)
...
def test_bmp280_functional():
    ...
    # Forced measurement -- the 6-byte read from 0xF7 may fail with a
    # bus-level error after many sequential transactions.  The device is
    # provably present (chip ID + calibration succeeded above), so a
    # bus-level I2CBackendError is acceptable here.
    try:
    ...
    # Full register map (128 bytes from 0x80)
    try:
        data = i2c.write_read(address=BMP280_ADDR, data=[0x80], num_bytes=128)
        ...
        _record("full register map (128 bytes)", passed,
                f"chip_id at offset {chip_id_offset}={hex(data[chip_id_offset]) if len(data) > chip_id_offset else 'N/A'}")
        if not passed:
            ok = False
    except Exception as e:
...
def test_output_formatting():
    ...
    # Scan output with devices (_format_scan_output returns only the grid;
    # the "Found N device(s)" summary is printed separately by scan())
    try:
    ...
    # Scan output empty - grid should have all "--" in scanned range
    try:
    ...
    # Scan output multiple devices - all addresses should appear in grid
    try:
...
def test_persist_params():
    ...
    # (We can't easily remove params in-place, so just verify setdefault works)
    try:
...
def test_config_persistence_functional():
    ...
    def _close_and_clear_cache():
        """Close cached drivers before clearing to avoid error -7 (device already open)."""
    ...
    try:
        config(I2C_NET, frequency_hz=400_000, pull_ups=True)
...
def test_config_edge_cases():
    ...
    # 32c. frequency_hz=10_000_000 (10MHz - way above Aardvark max)
    try:
        config(I2C_NET, frequency_hz=10_000_000)
        _record("config freq=10MHz", True,
                "no crash (Aardvark caps at ~800kHz)")
    except Exception as e:
        _record("config freq=10MHz", True,
                f"raises {type(e).__name__}: {e}")
...
def main():
    """Run all tests."""
    print("Aardvark I2C Comprehensive Test Suite")
