...
"""
Comprehensive SPI edge case tests targeting the Aardvark adapter on net 'spi1'.

Run with: lager python test_spi_aardvark.py --box <boxname>

Prerequisites:
- An SPI net configured in /etc/lager/saved_nets.json with instrument "aardvark_spi"
- Example net configuration:
  {
    "name": "spi1",
    "role": "spi",
    "instrument": "aardvark_spi",
    "params": {
      "port": 0,
      "mode": 0,
      "bit_order": "msb",
...
Optional: BMP280/BME280 sensor wired to SPI1 for device-specific tests.
"""
import sys
import os
import traceback
...
def test_imports():
    ...
    try:
        from lager.protocols.spi import AardvarkSPI
        _record("import AardvarkSPI", True)
    except Exception as e:
        _record("import AardvarkSPI", False, str(e))
        ok = False
...
# ---------------------------------------------------------------------------
def test_net_get():
    """Test Net.get('spi1', NetType.SPI) returns SPINet."""
    print("\n" + "=" * 60)
    print("TEST: Net.get")
    print("=" * 60)
...
def test_get_config():
    ...
    try:
        ...
        # 'params' key is optional -- LabJack nets have it, Aardvark nets
        # may store config at the top level or omit it entirely (the
        # dispatcher falls back to defaults).  Check whichever is present.
        ...
        if has_params:
            ...
            _record("config has 'params' key", True,
                    "not present (Aardvark uses defaults -- OK)")
...
def test_config_frequencies():
    """Test various frequencies: 100k, 500k, 1M, 4M, 8M."""
    print("\n" + "=" * 60)
    ...
    for freq in (100_000, 500_000, 1_000_000, 4_000_000, 8_000_000):
        try:
...
# ---------------------------------------------------------------------------
def test_read_basic():
    """Test read(n_words=1), read(n_words=4), read(n_words=100)."""
    print("\n" + "=" * 60)
...
def test_frequency_change():
    """Config 1MHz, read, config 4MHz, read -- both succeed."""
    print("\n" + "=" * 60)
    ...
    try:
        spi.config(frequency_hz=4_000_000)
        result = spi.read(n_words=4)
        passed = isinstance(result, list) and len(result) == 4
        _record("4MHz then read", passed, f"data={result}")
        if not passed:
            ok = False
    except Exception as e:
        _record("4MHz then read", False, str(e))
        ok = False
...
def test_large_transaction():
    """Read/write 1024 bytes (Aardvark supports up to 65535)."""
    print("\n" + "=" * 60)
...
def main():
    """Run all tests."""
    print("Aardvark SPI Comprehensive Test Suite")
