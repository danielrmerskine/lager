...
"""
Integration test suite for Joulescope JS220 CLI features.

Run via:
    lager python test/api/sensors/test_joulescope.py --box <box> -- <watt_net> [energy_net]

Arguments (after --):
    watt_net   - net configured with role 'watt-meter'
    energy_net - net configured with role 'energy-analyzer' (defaults to watt_net)

The same JS220 device can serve both roles under different net names.

Example:
    lager python test/integration/sensors/joulescope.py --box MY-BOX -- watt1 energy1
    lager python test/integration/sensors/joulescope.py --box MY-BOX -- pwr
"""
...
# ADC noise floor: values more negative than this are real failures.
# JS220 at near-zero current produces noise in the ±1e-6 range.
NOISE_FLOOR = -1e-6
...
if len(sys.argv) < 2:
    print("Usage: lager python joulescope.py --box <box> -- <watt_net> [energy_net]")
    sys.exit(1)
...
print("="*60)
print("  JOULESCOPE JS220 INTEGRATION TEST SUITE")
print("="*60)
...
def test_invalid_watt_net():
    try:
        Net.get("__nonexistent_js220_net__", type=NetType.WattMeter)
...
def test_invalid_energy_net():
    try:
        Net.get("__nonexistent_js220_net__", type=NetType.EnergyAnalyzer)
        failed("Expected error for invalid net name, got none")
    except Exception:
        passed("Invalid net name raised an error")
...
if watt_net is None:
    ...
    def test_read_consistent():
        ...
        # Both non-negative; if one is >0, ratio should be < 10x
        if v1 > 0 and v2 > 0:
...
if watt_net is None:
    ...
    def test_read_voltage():
        ...
        check(isinstance(val, float), "returns float")
        # Voltage can be negative on a floating/unloaded input — just verify it's finite
        check(abs(val) < 1000, f"voltage {val:.3f} V is within physical limits (±1000 V)")

    run_test("read_voltage() returns float within physical limits", test_read_voltage)
    ...
    def test_read_all_ranges():
        result = watt_net.read_all()
        # Current should be at or near zero (above noise floor)
        if "current" in result:
            check(result["current"] > NOISE_FLOOR,
                  f"result['current'] = {result['current']:.3e} A (above noise floor)")
        # Voltage can be negative on a floating/unloaded input
        if "voltage" in result:
            check(abs(result["voltage"]) < 1000,
                  f"result['voltage'] = {result['voltage']:.4f} V (within physical limits)")
        # Power = I * V, can be negative when voltage is negative
        if "power" in result:
            check(abs(result["power"]) < 1000,
                  f"result['power'] = {result['power']:.3e} W (within physical limits)")

    run_test("read_all() values are within physical limits", test_read_all_ranges)
...
if energy_net is None:
    ...
    run_test("read_stats() std is non-negative", test_stats_non_negative := test_stats_std_non_negative)


    def test_stats_voltage_plausible():
        """Voltage mean should be > 0 if device is powered."""
        result = energy_net.read_stats(duration=1.0)
        if "voltage" in result and "mean" in result["voltage"]:
            v = result["voltage"]["mean"]
            print(f"    voltage mean = {v:.4f} V")
            if v > 0:
                passed(f"Voltage mean {v:.4f} V > 0")
            else:
                skipped("Voltage mean is 0 (device may be unpowered)")

    run_test("read_stats() voltage mean is plausible", test_stats_voltage_plausible)
...
if watt_net is None or energy_net is None:
    ...
    def test_energy_charge_from_current():
        """Charge (C) over 1s ≈ mean current × 1s."""
        stats = energy_net.read_stats(duration=1.0)
        ...
        if i_mean is not None and charge_c is not None and i_mean > 1e-9:
            ...
            # Allow 20% tolerance — two separate acquisitions
            check(ratio < 0.20, f"charge_c ≈ mean_current × 1s (within 20%)")
        else:
            skipped("Current or charge near zero — cannot validate ratio")

    run_test("read_energy() charge_c ≈ read_stats() current_mean × duration", test_energy_charge_from_current)
...
# Close all cached Joulescope device handles so the process exits cleanly.
# Without this, background jsdrv threads keep the process alive after the
# script finishes, causing lager python to hang waiting for output.
