...
# ============================================================================
# I2C CLI Integration Tests - Aardvark
# ============================================================================
# Usage:
#   ./i2c_aardvark.sh <box> [net]
#
# Arguments:
#   box  - Box name or IP (e.g., <YOUR-BOX>)
#   net  - I2C net name (default: i2c1)
#
# Prerequisites:
#   - BMP280 connected to Aardvark I2C on box at address 0x76
#   - BMP280 powered: lager dac dac1 3.3 --box <YOUR-BOX>
#   - Net 'i2c1' configured (Aardvark I2C)
#
# Aardvark I2C characteristics:
#   - Maximum 65,535 bytes per transaction (no practical buffer limit)
#   - Hardware-controlled internal pull-ups (real, not silently ignored)
#   - Frequency range 1 kHz to 800+ kHz (via aa_i2c_bitrate)
#   - Note: 800kHz may fail with jumper wires due to parasitic capacitance;
#     works reliably up to ~790kHz on typical test setups
#   - Bus priming performed automatically after open
#   - Write NACK detection: cannot detect (silent failure)
# ============================================================================
...
if [ -z "$1" ]; then
    ...
    echo "  net  - I2C net name (default: i2c1)"
    echo ""
    echo "Prerequisites:"
    echo "  - BMP280 wired to Aardvark at address 0x76"
...
print_script_header "LAGER I2C CLI TEST SUITE - AARDVARK" "$BOX" "$NET"

echo "Device: BMP280 at $ADDR"
echo "Aardvark: no practical buffer limit, hardware pull-ups, 1kHz-800kHz+"
echo ""
...
# 1d. 800kHz (Aardvark high-speed)
echo -n "Test: Config 800kHz (Aardvark high-speed)... "
lager i2c "$NET" config --frequency 800k --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 1e. 1MHz (Aardvark may cap near 800kHz)
echo -n "Test: Config 1M (may cap at ~800kHz)... "
...
# 1g. 1kHz (Aardvark minimum)
echo -n "Test: Config 1kHz (Aardvark minimum)... "
lager i2c "$NET" config --frequency 1k --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 1h. Decimal frequency (3.5k)
echo -n "Test: Config 3.5k... "
lager i2c "$NET" config --frequency 3.5k --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 1i. Plain number frequency
echo -n "Test: Config 200000... "
lager i2c "$NET" config --frequency 200000 --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 1j. Pull-ups on (Aardvark: real hardware pull-ups)
echo -n "Test: Config --pull-ups on (hardware-controlled)... "
...
# 1l. Frequency only (no pull-ups flag)
echo -n "Test: Config frequency only... "
lager i2c "$NET" config --frequency 100k --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 1m. Pull-ups only (no frequency)
echo -n "Test: Config pull-ups only... "
lager i2c "$NET" config --pull-ups on --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 1n. Combined frequency + pull-ups
...
# 1o. No options (show current config)
...
# 3c. Read 56 bytes (LabJack max, Aardvark OK)
echo -n "Test: Read 56 bytes (OK)... "
lager i2c "$NET" read 56 --address "$ADDR" --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 3d. Read 57 bytes (Aardvark: MUST SUCCEED - no 56-byte limit)
echo -n "Test: Read 57 bytes (Aardvark: must succeed)... "
lager i2c "$NET" read 57 --address "$ADDR" --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 3e. Read 128 bytes (Aardvark large read)
echo -n "Test: Read 128 bytes... "
lager i2c "$NET" read 128 --address "$ADDR" --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 3f. Read 256 bytes (Aardvark large read)
echo -n "Test: Read 256 bytes... "
lager i2c "$NET" read 256 --address "$ADDR" --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"
...
# 3j. Read with frequency override
...
# 3k. Read from non-existent device
...
echo -ne '\xF4\x00' > /tmp/i2c_aa_test.bin
lager i2c "$NET" write --data-file /tmp/i2c_aa_test.bin --address "$ADDR" --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 4h. Write to non-existent device (Aardvark: cannot detect NACK on write)
echo -n "Test: Write to empty address 0x10 (NACK silent)... "
lager i2c "$NET" write 0x00 --address 0x10 --box "$BOX" >/dev/null 2>&1
# Aardvark write NACK is silent - both success and error acceptable
track_test "pass"
...
# 4n. Write 56 bytes (OK)
echo -n "Test: Write 56 bytes... "
...
# 4o. Write 57 bytes (Aardvark: MUST SUCCEED - no 56-byte limit)
echo -n "Test: Write 57 bytes (Aardvark: must succeed)... "
LONG_DATA=$(python3 -c "print('F4' + '00'*56)")
lager i2c "$NET" write "$LONG_DATA" --address "$ADDR" --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 4p. Write 128 bytes (Aardvark large write)
echo -n "Test: Write 128 bytes... "
LONG_DATA=$(python3 -c "print('F4' + '00'*127)")
lager i2c "$NET" write "$LONG_DATA" --address "$ADDR" --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 4q. BMP280 soft reset
...
# 4t. Write at 800kHz
echo -n "Test: Write --frequency 800k (Aardvark high-speed)... "
lager i2c "$NET" write 0xD0 --address "$ADDR" --frequency 800k --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"
...
# 5e. Transfer 56 bytes (OK)
echo -n "Test: Transfer 56 bytes... "
lager i2c "$NET" transfer 56 --address "$ADDR" --data 0x80 --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 5f. Transfer 57 bytes (Aardvark: MUST SUCCEED - no 56-byte limit)
echo -n "Test: Transfer 57 bytes (Aardvark: must succeed)... "
lager i2c "$NET" transfer 57 --address "$ADDR" --data 0x80 --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 5g. Transfer 128 bytes (Aardvark large transfer)
echo -n "Test: Transfer 128 bytes... "
lager i2c "$NET" transfer 128 --address "$ADDR" --data 0x80 --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 5h. Transfer 256 bytes (Aardvark large transfer)
echo -n "Test: Transfer 256 bytes... "
lager i2c "$NET" transfer 256 --address "$ADDR" --data 0x80 --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 5i. Hex output format
...
# 5l. Transfer with frequency override
...
# 5m. Transfer to non-existent device
...
# 5n. Transfer without --data (just read)
...
# 5p. Transfer with multi-byte write data
...
# 5q. Transfer with data-file
echo -n "Test: Transfer with --data-file... "
echo -ne '\xD0' > /tmp/i2c_aa_reg.bin
lager i2c "$NET" transfer 1 --address "$ADDR" --data-file /tmp/i2c_aa_reg.bin --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"
...
# 9g. Pull-ups toggle between commands
echo -n "Test: Pull-ups on -> transfer -> pull-ups off -> transfer... "
PULL_OK=true
lager i2c "$NET" config --pull-ups on --box "$BOX" >/dev/null 2>&1
OUTPUT=$(lager i2c "$NET" transfer 1 --address "$ADDR" --data "$CHIP_ID_REG" --box "$BOX" 2>&1)
if ! echo "$OUTPUT" | grep -q "$CHIP_ID_VAL"; then PULL_OK=false; fi
...
# 9h. Large transfer at 400kHz
echo -n "Test: 128-byte transfer at 400kHz... "
OUTPUT=$(lager i2c "$NET" transfer 128 --address "$ADDR" --data 0x80 --frequency 400k --box "$BOX" 2>&1)
if [ $? -eq 0 ]; then
    track_test "pass"
else
    track_test "fail"
...
# ============================================================================
# 11. AARDVARK-SPECIFIC TESTS
# ============================================================================
start_section "Aardvark-Specific"
print_section_header "SECTION 11: AARDVARK-SPECIFIC TESTS"

# 11a. Pull-ups cycle: on -> verify -> off -> verify -> on
echo -n "Test: Pull-ups cycle (on/off/on)... "
CYCLE_OK=true
lager i2c "$NET" config --pull-ups on --box "$BOX" >/dev/null 2>&1
OUTPUT=$(lager i2c "$NET" config --box "$BOX" 2>&1)
if ! echo "$OUTPUT" | grep -qi "pull_ups=on"; then CYCLE_OK=false; fi
lager i2c "$NET" config --pull-ups off --box "$BOX" >/dev/null 2>&1
OUTPUT=$(lager i2c "$NET" config --box "$BOX" 2>&1)
if ! echo "$OUTPUT" | grep -qi "pull_ups=off"; then CYCLE_OK=false; fi
lager i2c "$NET" config --pull-ups on --box "$BOX" >/dev/null 2>&1
OUTPUT=$(lager i2c "$NET" config --box "$BOX" 2>&1)
if ! echo "$OUTPUT" | grep -qi "pull_ups=on"; then CYCLE_OK=false; fi
...
# 11b. 512-byte transfer (Aardvark large buffer)
echo -n "Test: Transfer 512 bytes (Aardvark large buffer)... "
lager i2c "$NET" transfer 512 --address "$ADDR" --data 0x80 --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 11c. 400kHz chip ID read (fast mode)
echo -n "Test: Chip ID at 400kHz (fast mode)... "
OUTPUT=$(lager i2c "$NET" transfer 1 --address "$ADDR" --data "$CHIP_ID_REG" --frequency 400k --box "$BOX" 2>&1)
if echo "$OUTPUT" | grep -q "$CHIP_ID_VAL"; then
    track_test "pass"
else
...
# 11d. 1kHz chip ID read (Aardvark low-speed)
echo -n "Test: Chip ID at 1kHz (Aardvark minimum)... "
OUTPUT=$(lager i2c "$NET" transfer 1 --address "$ADDR" --data "$CHIP_ID_REG" --frequency 1k --box "$BOX" 2>&1)
if echo "$OUTPUT" | grep -q "$CHIP_ID_VAL"; then
    track_test "pass"
else
    track_test "fail"
...
# 11e. Full speed range sweep (800kHz excluded: fails with jumper wires)
echo -n "Test: Speed sweep (1k, 10k, 100k, 400k)... "
SWEEP_OK=true
for freq in 1k 10k 100k 400k; do
    OUTPUT=$(lager i2c "$NET" transfer 1 --address "$ADDR" --data "$CHIP_ID_REG" --frequency "$freq" --box "$BOX" 2>&1)
    if ! echo "$OUTPUT" | grep -q "$CHIP_ID_VAL"; then
        SWEEP_OK=false
        echo "  (failed at $freq)"
...
print_section_header "SECTION 12: CLEANUP"

# 12a. Restore BMP280 to sleep mode
echo -n "Test: Restore BMP280 sleep mode... "
lager i2c "$NET" write 0xF400 --address "$ADDR" --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 12b. Restore default config
echo -n "Test: Restore default config... "
lager i2c "$NET" config --frequency 100k --pull-ups on --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 12c. Clean up temp files
echo -n "Test: Clean up temp files... "
rm -f /tmp/i2c_aa_test.bin /tmp/i2c_aa_reg.bin
track_test "pass"
