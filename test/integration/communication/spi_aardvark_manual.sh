...
# ============================================================================
# SPI CLI Integration Tests - Aardvark Manual CS Mode
# ============================================================================
# Usage:
#   ./spi_aardvark_manual.sh <box>
#
# Arguments:
...
#   - Net 'spi1' configured with cs_mode=manual
#   - Net 'gpio16' configured as GPIO output
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo "Prerequisites:"
    echo "  - BMP280 wired to Aardvark SPI (manual CS via gpio16)"
...
print_script_header "LAGER SPI CLI TEST SUITE - AARDVARK MANUAL CS" "$BOX" "$SPI_NET (CS: $GPIO_CS)"

echo "Device: BMP280 at SPI (manual CS via $GPIO_CS)"
...
echo -n "Test: Config mode 0, 1M, msb, 8-bit... "
...
#     MISO floats without pull-up, so response varies (00 00 or ff ff);
#     the key invariant is that the chip ID (58) must NOT appear.
...
echo -ne '\xD0\x00' > /tmp/spi_manual_test.bin
lager gpo "$GPIO_CS" low --box "$BOX" >/dev/null 2>&1
OUTPUT=$(lager spi "$SPI_NET" transfer --data-file /tmp/spi_manual_test.bin 2 --box "$BOX" 2>&1)
lager gpo "$GPIO_CS" high --box "$BOX" >/dev/null 2>&1
if echo "$OUTPUT" | grep -qi "$CHIP_ID_VAL"; then
    track_test "pass"
else
...
echo -n "Test: Restore config (mode 0, 1M, msb, 8-bit, cs-active low)... "
lager spi "$SPI_NET" config --mode 0 --frequency 1M --bit-order msb --word-size 8 --cs-active low --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 10b. Set CS high (deasserted)
echo -n "Test: Set CS high (deasserted)... "
lager gpo "$GPIO_CS" high --box "$BOX" >/dev/null 2>&1 && track_test "pass" || track_test "fail"

# 10c. Restore BMP280 sleep mode
...
# 10d. Clean temp files
echo -n "Test: Clean up temp files... "
rm -f /tmp/spi_manual_test.bin
track_test "pass"
